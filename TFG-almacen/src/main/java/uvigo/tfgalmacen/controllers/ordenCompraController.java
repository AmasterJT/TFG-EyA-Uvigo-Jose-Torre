...
public class ordenCompraController implements Initializable {
    ...
    private void agregarItemFXML() {
        final String proveedorSel = combo_proveedor_oc.getSelectionModel().getSelectedItem();
        final String productoSel = combo_producto_oc.getSelectionModel().getSelectedItem();

        final boolean proveedorOK = seleccionValida(proveedorSel, PLACEHOLDER_PROVEEDOR);
        final boolean productoOK = seleccionValida(productoSel, PLACEHOLDER_PRODUCTO);
        ...
        try {
            ...
            // En el futuro: ItemOrdenCompraController ctrl = loader.getController(); // setData(...)
